...
# ``universal-3-5-pro`` (and any ``universal-3-5-pro-*`` release) is the current
# family; the ``u3-rt-pro`` names are legacy aliases AssemblyAI redirects to it.
...
def is_u3_pro_model(model: str | None | NotGiven) -> bool:
    ...
    Matches ``universal-3-5-pro`` (and any ``universal-3-5-pro-*`` variant) plus
    the legacy ``u3-rt-pro`` aliases, so U3 Pro-only features are gated on the
